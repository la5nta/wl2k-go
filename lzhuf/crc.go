...
type crcWriter struct{ sum crc16 }

func udpCRC16(cp int, sum crc16) crc16 {
	return ((sum << 8) & 0xff00) ^ crc16tab[((sum>>8)&0xff)] ^ crc16(cp)
}
...
func (w *crcWriter) Write(p []byte) (int, error) {
	for _, c := range p {
		w.sum = udpCRC16(int(c), w.sum)
	}
	return len(p), nil
}

func (w *crcWriter) Sum() crc16 {
	sum := w.sum
	for _, c := range []byte{0, 0} {
		sum = udpCRC16(int(c), sum)
	}
	return sum
}

func crc(p []byte) crc16 {
	var sum crc16
	for _, c := range append(p, 0, 0) {
		sum = udpCRC16(int(c), sum)
	}
	return sum
}
